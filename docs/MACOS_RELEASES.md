...
loads them. Keep app verification values in
`apps/macos/config/.env.release.<environment>` instead.

Staging and production must use different Sparkle Keychain accounts and
different Sparkle/P-256 public keys. The two checked-in public trust files are
an auditable policy, not secrets. `scripts/macos-app` and
