...
The environment-specific `.env.release.staging` and `.env.release.production`
files are committed because they contain only the public Sparkle Ed25519 key,
public P-256 manifest key, Apple Team ID, and a positive globally increasing
build sequence. `.env.release.example` documents the shape. The corresponding
private signing keys never belong in this repository or the macOS application.
